...
if %DEMO_ARG% == "demo" (
  echo Making Fz10m Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Fz10m Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Fz10m-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild Fz10m.sln /t:Fz10m-app;Fz10m-vst3;Fz10m-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild Fz10m.sln /t:Fz10m-app;Fz10m-vst3;Fz10m-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Fz10m.aaxplugin\Contents\Win32\Fz10m.aaxplugin --out .\build-win\aax\bin\Fz10m.aaxplugin\Contents\Win32\Fz10m.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Fz10m.aaxplugin\Contents\x64\Fz10m.aaxplugin --out .\build-win\aax\bin\Fz10m.aaxplugin\Contents\x64\Fz10m.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Fz10m.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Fz10m.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Fz10m Installer" ".\installer\Fz10m Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Fz10m Installer.exe" ".\installer\Fz10m Demo Installer.exe"
  REM -del ".\installer\Fz10m Installer.exe"
  REM -)
