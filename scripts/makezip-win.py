...
def main():
  ...
  vst3_bundle = build_dir + "\\Fz10m.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\Fz10m Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\Fz10m Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\Fz10m manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\Fz10m.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "Fz10m.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\Fz10m_x64.exe",
      projectpath + "\\build-win\\Fz10m_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\Fz10m_x64.clap", "Fz10m_x64.clap"),
      (projectpath + "\\build-win\\Fz10m_ARM64EC.clap", "Fz10m_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\Fz10m.clap", "Fz10m_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\Fz10m.clap", "Fz10m_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Fz10m-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Fz10m-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Fz10m-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Fz10m-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Fz10m-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Fz10m-clap_ARM64EC.pdb"
  ]
